// app/api/chickadee/[id]/route.ts
import { NextRequest, NextResponse } from 'next/server';
import type { CKWCProduct, CKProduct, CKApiError } from '@/lib/chikadee.types';
...
async function fetchReviews(productId: number): Promise<WCReview[]> {
  try {
    const res = await fetch(
      `${CK_BASE}/wp-json/wc/v3/products/${productId}/reviews?per_page=5`,
...
): Promise<NextResponse<{ product: CKProduct; reviews: WCReview[] } | CKApiError>> {
  try {
    const { id } = await params;
    const isNumeric = /^\d+$/.test(id);

    let raw: CKWCProduct | null = null;
    ...
    if (!raw) {
      ...
      if (res.ok) {
        const arr: CKWCProduct[] = await res.json();
        raw = arr[0] ?? null;
      }
    }
    ...
    console.error('[chickadee single]', msg);
