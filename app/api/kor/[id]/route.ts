// app/api/kor/[id]/route.ts
import { NextRequest, NextResponse } from 'next/server';
import type { KORWCProduct, KORProduct, KORApiError } from '@/lib/kor.types';
...
async function fetchReviews(productId: number): Promise<WCReview[]> {
  try {
    const res = await fetch(
      `${KOR_BASE}/wp-json/wc/v3/products/${productId}/reviews?per_page=5`,
...
): Promise<NextResponse<{ product: KORProduct; reviews: WCReview[] } | KORApiError>> {
  try {
    const { id } = await params;
    const isNumeric = /^\d+$/.test(id);
    let raw: KORWCProduct | null = null;
    ...
    if (!raw) {
      ...
      if (res.ok) {
        const arr: KORWCProduct[] = await res.json();
        raw = arr[0] ?? null;
      }
    }
