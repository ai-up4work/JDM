// app/api/otaku/[id]/route.ts
import { NextRequest, NextResponse } from 'next/server';
import type { OTKWCProduct, OTKProduct, OTKApiError } from '@/lib/otaku.types';
...
async function fetchReviews(productId: number): Promise<WCReview[]> {
  try {
    const res = await fetch(
      `${OTK_BASE}/wp-json/wc/v3/products/${productId}/reviews?per_page=5`,
...
): Promise<NextResponse<{ product: OTKProduct; reviews: WCReview[] } | OTKApiError>> {
  try {
    const { id } = await params;
    const isNumeric = /^\d+$/.test(id);
    let raw: OTKWCProduct | null = null;
    ...
    if (!raw) {
      ...
      if (res.ok) {
        const arr: OTKWCProduct[] = await res.json();
        raw = arr[0] ?? null;
      }
    }
    ...
    console.error('[otaku single]', msg);
