// app/api/skye/[id]/route.ts
import { NextRequest, NextResponse } from 'next/server';
import type { SkyeWCProduct, SkyeProduct, SkyeApiError } from '@/lib/skye.types';
...
async function fetchReviews(productId: number): Promise<WCReview[]> {
  try {
    const res = await fetch(
      `${SKYE_BASE}/wp-json/wc/v3/products/${productId}/reviews?per_page=5`,
...
): Promise<NextResponse<{ product: SkyeProduct; reviews: WCReview[] } | SkyeApiError>> {
  try {
    const { id } = await params;
    const isNumeric = /^\d+$/.test(id);

    let raw: SkyeWCProduct | null = null;
    ...
    if (!raw) {
      ...
      if (res.ok) {
        const arr: SkyeWCProduct[] = await res.json();
        raw = arr[0] ?? null;
      }
    }
