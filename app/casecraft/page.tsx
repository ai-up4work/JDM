...
function toLKR(usd: number) {
  return Math.round(usd * 320); // update rate here as needed
}
...
export default function CustomCasePage() {
  ...
  // Step 2 — model
  // brands list is derived from the selected case type's mockupFolder
  ...
  // Ensure the active brand tab is valid for this case type's brand list
  const activeBrand = getBrand(brands, brandId)
    ?? brands[0];                        // fall back to first available brand

  // Only show brand tabs that exist in the current series
  const availableBrandTabs = useMemo(() => {
    // If the case type restricts brands (e.g. iphone-only for MagSafe), filter further
  ...
  // Filtered model list within the selected brand
  ...
  const selectCaseType = (ct: CaseType) => {
    ...
    setModel(null);
    setSearch('');
    // Get the brand list for this case type and default to iphone if available
    const nextBrands = getBrandsForFolder(ct.mockupFolder);
    const nextBrandIds = nextBrands.map(b => b.id);
    if (!nextBrandIds.includes(brandId)) {
      setBrandId(nextBrandIds[0] as BrandId);
    }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_400px] gap-8 xl:gap-16 items-start">
            ...
            <div className={`rounded-2xl border transition-all duration-300 overflow-hidden
              ...
              {step === 2 && (
                <div className="border-t border-border">
                  ...
                  {/* Brand tabs — driven by getBrandsForFolder, scoped to compatible brands */}
          ...
          <div className="lg:sticky lg:top-20 flex flex-col items-center gap-5">
            ...
            >
              ...
              {/* Case overlay — shown as soon as a model is selected, hides placeholder */}
              {model && (
              ...
              {/* Placeholder — only shown when no model selected yet */}
              {!model && (
