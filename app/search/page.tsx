...
// useSearchParams lives here — inside <Suspense> in the default export
  ...
  return (
    <div className="w-full px-10 py-8">
      ...
      <div className="mb-6">
        {query ? (
          <>
            <p className="text-sm text-muted-foreground mb-1">{filtered.length} results for</p>
            ...
            <Link key={product.id} href={`/product/${product.id}`} className="group flex flex-col gap-2">
              ...
              <div className="flex items-start justify-between gap-2 px-0.5">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-muted-foreground truncate mt-0.5">{product.brand} • {product.name}</p>
                  <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                    {(product as any).express ? (
                      <span className="flex items-center gap-1 bg-blue-50 text-blue-600 text-xs font-semibold px-2 py-0.5 rounded-full border border-blue-200">⚡ Express</span>
                    ) : (
                      <span className="flex items-center gap-1 bg-gray-50 text-gray-500 text-xs font-medium px-2 py-0.5 rounded-full border border-gray-200">➜ 7 Days</span>
                    )}
...
function SearchSkeleton() {
  return (
    <div className="w-full px-10 py-8">
      ...
      <div className="h-9 w-64 bg-muted rounded-lg animate-pulse mb-8" />
...
// Default export — useSearchParams is ONLY called inside SearchContent
// which is wrapped in Suspense here, satisfying Next.js prerender requirement
