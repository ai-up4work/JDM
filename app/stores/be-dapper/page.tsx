...
import {
  ...
  X, ArrowUpDown, Truck, RefreshCw, Shield,
  AlertCircle, Search,
} from 'lucide-react';
import type { BDProduct, BDApiResponse } from '@/lib/bedapper.types';
...
// ─── Product Card — links to PDP ─────────────────────────────────────────────

function ProductCard({ product }: { product: BDProduct }) {
  ...
  return (
    <div className="group flex flex-col">
      {/* Image — full link to PDP */}
      <Link
        href={`/stores/be-dapper/${product.slug}`}
      ...
      >
        ...
        {/* Wishlist — preventDefault so it doesn't navigate */}
        <button
          type="button"
...
type CartItem = BDProduct & { selectedSize?: string | null; selectedColor?: string | null; qty: number };

function MiniCart({ items, onClose, onClear }: { items: CartItem[]; onClose: () => void; onClear: () => void }) {
  const total = items.reduce((s, i) => s + i.price * i.qty, 0);

  const whatsappText = encodeURIComponent(
    `Hi, I'd like to order the following from Be Dapper:\n\n` +
    items.map(i =>
      `• ${i.name}${i.selectedSize ? ` (${i.selectedSize})` : ''}${i.selectedColor ? ` — ${i.selectedColor}` : ''} × ${i.qty} — Rs ${(i.price * i.qty).toLocaleString()}`
    ).join('\n') +
        ...
        {items.length > 0 && (
          <div className="p-4 border-t border-border space-y-3">
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm font-bold text-foreground">Rs {total.toLocaleString()}</span>
            </div>
            <a
...
export default function BeDapperPage() {
  ...
  // Cart lives in sessionStorage — shared with PDP page
  ...
  // Sync cart from sessionStorage on mount (items may have been added from PDP)
  useEffect(() => {
    setCart(readCart());
    // Also listen for storage events in case PDP adds items in the same tab
    const sync = () => setCart(readCart());
    window.addEventListener('bd_cart_updated', sync);
    return () => window.removeEventListener('bd_cart_updated', sync);
  }, []);
  ...
  const fetchProducts = useCallback(async (cat: string, pg: number, q: string) => {
    ...
    try {
      ...
      const res = await fetch(`/api/bedapper?${params}`, { signal: abortRef.current.signal });
      if (!res.ok) {
        const err = await res.json().catch(() => ({}));
        throw new Error(err.error ?? `HTTP ${res.status}`);
      }

      const data: BDApiResponse = await res.json();
      ...
      <div className="sticky top-0 z-30 bg-background/80 backdrop-blur-md mt-4">
        <div className="max-w-7xl mx-auto h-14 flex items-center justify-between gap-4 px-4 sm:px-10 lg:px-40">
          <div className="flex items-center gap-2 text-xs text-muted-foreground shrink-0">
            ...
            <ChevronRight size={11} />
            <span className="text-foreground font-medium">Be Dapper</span>
          </div>
        ...
        <div className="max-w-7xl mx-auto py-8 sm:py-12 min-w-0">
          ...
          <div className="mb-10 sm:mb-14">
            ...
            />
              ...
              <div>
                <div className="flex items-center gap-2">
                  <h1 className="text-lg font-black text-foreground tracking-tight">BE DAPPER</h1>
            ...
            <p className="text-muted-foreground text-sm max-w-md mb-6">
              Premium men's shirts, polos and essentials. Products pulled live from Be Dapper's full catalogue — always up to date.
            </p>
              ...
              <div className="relative flex-1 sm:flex-none sm:w-64">
                ...
                <input
                  ...
                  className="w-full pl-8 pr-3 py-2 text-xs rounded-xl border border-border bg-background
                    ...
                    focus:ring-foreground/10 focus:border-foreground/30 transition-all"
                />
              </div>
          ...
          <div className="mt-16 pt-8 border-t border-border grid grid-cols-1 sm:grid-cols-3 gap-8">
            {[
              { title: 'About Be Dapper', body: 'Original Be Dapper products resold locally. Same authentic quality, no courier wait — pick up or get delivered within your area. Products synced live from bedapper.lk.' },
              { title: 'Delivery',        body: 'Island-wide shipping via courier in 2–4 working days. Local same-day delivery available in our area. Cash on delivery accepted.'  },
              { title: 'How it works',    body: 'Browse here, add to bag, and place your order via WhatsApp. We confirm stock with Be Dapper and dispatch to you directly.'         },
            ].map(s => (
              <div key={s.title}>
