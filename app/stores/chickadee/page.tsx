...
function ProductCard({ product }: { product: ChickadeeProduct }) {
  ...
  return (
    <div className="group flex flex-col">
      <Link href={`/stores/chickadee/${product.slug}`}
      ...
      <Link href={`/stores/chickadee/${product.slug}`} className="flex flex-col flex-1 hover:opacity-75 transition-opacity">
...
function MiniCart({ items, onClose, onClear }: { items: CartItem[]; onClose: () => void; onClear: () => void }) {
  ...
  const whatsappText = encodeURIComponent(
    `Hi Chickadee! 💍 I'd like to order the following:\n\n` +
    items.map(i => `• ${i.name} × ${i.qty} — ${fmtPrice(i.price * i.qty)}`).join('\n') +
          ...
          <div className="p-4 border-t border-border space-y-3">
            ...
            {total >= 10000 && (
              <p className="text-[10px] text-emerald-600 dark:text-emerald-400 text-center font-medium">
                🎉 Free shipping unlocked (orders above LKR 10,000)
              </p>
            )}
...
export default function ChickadeePage() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('chickadee_cart_updated', sync);
    return () => window.removeEventListener('chickadee_cart_updated', sync);
  }, []);
  ...
  const fetchProducts = useCallback(async (cat: string, pg: number, q: string) => {
    ...
    try {
      ...
      const res = await fetch(`/api/chickadee?${params}`, { signal: abortRef.current.signal });
      if (!res.ok) {
      ...
      <div className="sticky top-0 z-30 bg-background/80 backdrop-blur-md mt-4">
        <div className="max-w-7xl mx-auto h-14 flex items-center justify-between gap-4 px-4 sm:px-10 lg:px-40">
          <div className="flex items-center gap-2 text-xs text-muted-foreground shrink-0">
            ...
            <ChevronRight size={11} />
            <span className="text-foreground font-medium">Chickadee</span>
          </div>
        ...
        <div className="max-w-7xl mx-auto py-8 sm:py-12 min-w-0">
          ...
          <div className="mb-10 sm:mb-14">
            ...
            <p className="text-3xl sm:text-4xl font-bold text-foreground tracking-tight leading-tight mb-3 max-w-lg">
              Bold, minimalistic<br />gold plated jewellery.
            </p>
            <p className="text-muted-foreground text-sm max-w-md mb-6">
              Premium imitation jewellery — earrings, necklaces, rings, bracelets and more.
