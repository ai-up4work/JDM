// app/stores/enzayn-ceylon/[slug]/page.tsx
'use client';
...
import {
  ...
  ShoppingBag, Heart, Star, Check, Truck, Palette,
  ExternalLink, ArrowLeft, AlertCircle, Share2, ZoomIn,
  Minus, Plus, Shirt,
} from 'lucide-react';
import type { ENZProduct } from '@/lib/enzayn.types';
...
export default function EnzaynProductPage() {
  const { slug }                            = useParams<{ slug: string }>();
  const [product,    setProduct]            = useState<ENZProduct | null>(null);
  ...
  useEffect(() => {
    ...
    update();
    window.addEventListener('enzayn_cart_updated', update);
    return () => window.removeEventListener('enzayn_cart_updated', update);
  }, []);

  useEffect(() => {
    if (!slug) return;
    setLoading(true); setError(null);
    fetch(`/api/enzayn/${encodeURIComponent(slug)}`)
      ...
      <div className="sticky top-0 z-30 bg-background/80 backdrop-blur-md mt-4">
        <div className="max-w-7xl mx-auto h-14 flex items-center justify-between gap-4 px-4 sm:px-10 lg:px-40">
          <div className="flex items-center gap-2 text-xs text-muted-foreground shrink-0 min-w-0">
            ...
            <ChevronRight size={11} className="shrink-0" />
            <Link href="/stores/enzayn-ceylon" className="hover:text-foreground transition-colors font-medium shrink-0">Enzayn Ceylon</Link>
            {product && (
              <>
                <ChevronRight size={11} className="shrink-0" />
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <Link href="/stores/enzayn-ceylon"
          ...
          {error && !loading && (
            <div className="flex items-start gap-3 p-5 rounded-2xl border border-red-200 dark:border-red-900/50 bg-red-50 dark:bg-red-950/20 max-w-lg">
              ...
              <div>
                ...
                <Link href="/stores/enzayn-ceylon" className="text-xs font-semibold text-foreground underline hover:no-underline">← Back</Link>
              </div>
            </div>
          )}
            ...
            <>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 xl:gap-16 items-start">
                ...
                <div className="flex flex-col gap-5 py-1">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {[
                      { icon: <Palette size={14} />, label: '20+ colours',   sub: 'Wide range'           },
                      { icon: <Truck size={14} />,   label: 'Island-wide',   sub: '1–2 working days'     },
                      { icon: <Shirt size={14} />,   label: 'Sri Lankan',    sub: 'Local brand'          },
                    ].map(b => (
                  ...
                  <a href={product.permalink} target="_blank" rel="noopener noreferrer"
                    ...
                    {/* View original listing on enzaynceylon.com */}
                  </a>
                </div>
              </div>
                  ...
                  {reviews.length > 0 ? (
                    <div>
                      ...
                      <a href={`${product.permalink}#reviews`} target="_blank" rel="noopener noreferrer"
                        ...
                        <ExternalLink size={11} /> See all reviews on enzaynceylon.com
                      </a>
                    </div>
                  ) : (
              ...
              <div className="mt-14 p-5 rounded-2xl bg-secondary/30 border border-border">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  {[
                    { step: '01', title: 'You order here',      body: 'Add to bag and place your order via WhatsApp. We confirm your size and colour with Enzayn Ceylon.' },
                    { step: '02', title: 'We bulk-buy for you', body: 'We collect orders and purchase from Enzayn Ceylon — no courier wait, delivered to us in 1–2 days.' },
                    { step: '03', title: 'We deliver locally',  body: 'Your item arrives with us and we deliver to your door the same or next day in your area.' },
                  ].map(s => (
