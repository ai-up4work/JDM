// app/stores/enzayn-ceylon/page.tsx
'use client';
...
import {
  ...
  X, ArrowUpDown, Truck, Palette, AlertCircle, Search, Shirt,
} from 'lucide-react';
import type { ENZProduct, ENZApiResponse } from '@/lib/enzayn.types';
...
const CATEGORIES = [
  { key: '',                  label: 'All'            },
  { key: 'polo',              label: 'Polo'           },
  { key: 'short-sleeve-polo', label: 'Polo Short'     },
  { key: 'long-sleeve-polo',  label: 'Polo Long'      },
  { key: 'crew-neck',         label: 'Crew Neck'      },
  { key: 'short-sleeve',      label: 'Short Sleeve'   },
  { key: 'long-sleeve',       label: 'Long Sleeve'    },
  { key: 'shoulder-drop',     label: 'Drop Shoulder'  },
  { key: 'accessories',       label: 'Accessories'    },
  { key: 'caps',              label: 'Caps'           },
  { key: 'back-packs',        label: 'Backpacks'      },
  { key: 'hand-bands',        label: 'Hand Bands'     },
];
...
// Prices stored as USD cents
function fmtPrice(cents: number) {
  return `$${(cents / 100).toFixed(2)}`;
}

const CART_KEY = 'enzayn_cart';
type CartItem = ENZProduct & { selectedSize?: string | null; selectedColor?: string | null; qty: number };
...
function writeCart(items: CartItem[]) {
  ...
  window.dispatchEvent(new Event('enzayn_cart_updated'));
}
...
function ProductCard({ product }: { product: ENZProduct }) {
  ...
  return (
    <div className="group flex flex-col">
      <Link href={`/stores/enzayn-ceylon/${product.slug}`}
      ...
      <Link href={`/stores/enzayn-ceylon/${product.slug}`} className="flex flex-col flex-1 hover:opacity-75 transition-opacity">
...
function MiniCart({ items, onClose, onClear }: { items: CartItem[]; onClose: () => void; onClear: () => void }) {
  ...
  const whatsappText = encodeURIComponent(
    `Hi Enzayn Ceylon! 👕 I'd like to order:\n\n` +
    items.map(i =>
...
export default function EnzaynCeylonPage() {
  ...
  const [products,    setProducts]    = useState<ENZProduct[]>([]);
  ...
  useEffect(() => {
    ...
    window.addEventListener('enzayn_cart_updated', sync);
    return () => window.removeEventListener('enzayn_cart_updated', sync);
  }, []);
  ...
  const fetchProducts = useCallback(async (cat: string, pg: number, q: string) => {
    ...
    try {
      ...
      const res = await fetch(`/api/enzayn?${params}`, { signal: abortRef.current.signal });
      if (!res.ok) {
        const err = await res.json().catch(() => ({}));
        throw new Error(err.error ?? `HTTP ${res.status}`);
      }
      const data: ENZApiResponse = await res.json();
      ...
      <div className="sticky top-0 z-30 bg-background/80 backdrop-blur-md mt-4">
        <div className="max-w-7xl mx-auto h-14 flex items-center justify-between gap-4 px-4 sm:px-10 lg:px-40">
          <div className="flex items-center gap-2 text-xs text-muted-foreground shrink-0">
            ...
            <ChevronRight size={11} />
            <span className="text-foreground font-medium">Enzayn Ceylon</span>
          </div>
      ...
      <div className="w-full min-w-0 px-4 sm:px-10 lg:px-40">
        <div className="max-w-7xl mx-auto py-8 sm:py-12 min-w-0">
          ...
          <div className="mb-10 sm:mb-14">
            <div className="flex items-center gap-3 mb-5">
              <div className="w-10 h-10 rounded-xl overflow-hidden shrink-0">
                <img src="/store-icon/enzayn-ceylon.png" alt="Enzayn Ceylon"
              ...
              <div>
                <div className="flex items-center gap-2">
                  <h1 className="text-lg font-black text-foreground tracking-tight">ENZAYN CEYLON</h1>
                ...
                <p className="text-[10px] text-muted-foreground">Real-time stock from enzaynceylon.com · Ships island-wide</p>
              </div>
            </div>

            <p className="text-3xl sm:text-4xl font-bold text-foreground tracking-tight leading-tight mb-3 max-w-lg">
              Premium polos &<br />crew necks. 20+ colours.
            </p>
            <p className="text-muted-foreground text-sm max-w-md mb-6">
              Sri Lankan clothing brand — classic polos, crew necks, drop shoulders and accessories in 20+ colours. Live catalogue from Enzayn Ceylon.
            </p>

            <div className="flex items-center gap-6 flex-wrap">
              {[
                { icon: <Palette size={13} />, label: '20+ colour collections'    },
                { icon: <Shirt size={13} />,   label: 'Polos & crew necks'        },
                { icon: <Truck size={13} />,   label: 'Free ship over Rs. 7,500'  },
              ].map(b => (
          ...
          <div className="flex items-center justify-between gap-3 mb-6 flex-wrap">
            <div className="flex items-center gap-2 flex-1 min-w-0">
              <div className="relative flex-1 sm:flex-none sm:w-64">
                ...
                <input type="text" placeholder="Search Enzayn…" value={searchInput}
          ...
          <div className="mt-16 pt-8 border-t border-border grid grid-cols-1 sm:grid-cols-3 gap-8">
            {[
              { title: 'About Enzayn Ceylon', body: 'Original Enzayn Ceylon pieces resold locally. Sri Lankan brand known for premium polos and crew necks in 20+ vibrant colours — island-wide delivery in 1–2 days.' },
              { title: 'Free delivery',       body: 'Free island-wide delivery on orders over Rs. 7,500. Dispatched within 1–2 working days. Cash on delivery available.' },
              { title: 'How it works',        body: 'Browse here, add to bag, order via WhatsApp. We confirm stock with Enzayn and deliver locally — same or next day in your area.' },
            ].map(s => (
              <div key={s.title}>
