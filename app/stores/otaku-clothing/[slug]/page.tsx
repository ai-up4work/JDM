// app/stores/otaku-clothing/[slug]/page.tsx
'use client';
...
function ImageGallery({ images, title }: { images: string[]; title: string }) {
  ...
  if (!images.length) return (
    <div className="aspect-[3/4] rounded-2xl bg-secondary/40 flex items-center justify-center text-muted-foreground/20">
      <Shirt size={48} />
    </div>
  );
...
export default function OtakuProductPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/otaku/${encodeURIComponent(slug)}`)
      ...
      <div className="sticky top-0 z-30 bg-background/80 backdrop-blur-md mt-4">
        <div className="max-w-7xl mx-auto h-14 flex items-center justify-between gap-4 px-4 sm:px-10 lg:px-40">
          <div className="flex items-center gap-2 text-xs text-muted-foreground shrink-0 min-w-0">
            ...
            <ChevronRight size={11} className="shrink-0" />
            <Link href="/stores/otaku-clothing" className="hover:text-foreground transition-colors font-medium shrink-0">Otaku Clothing</Link>
            {product && (
              <>
                <ChevronRight size={11} className="shrink-0" />
          ...
          {error && !loading && (
            <div className="flex items-start gap-3 p-5 rounded-2xl border border-red-200 dark:border-red-900/50 bg-red-50 dark:bg-red-950/20 max-w-lg">
              ...
              <div>
                ...
                <Link href="/stores/otaku-clothing" className="text-xs font-semibold text-foreground underline hover:no-underline">← Back</Link>
              </div>
            </div>
          )}
                ...
                <div>
                  ...
                  {(product.sizes.length > 0 || product.colors.length > 0) && (
                    <div className="mt-5 space-y-2">
                      ...
                      <div className="flex gap-3 text-xs">
                        <span className="font-semibold text-foreground w-20 shrink-0">Franchise</span>
                        <span className="text-muted-foreground">{product.franchiseLabel}</span>
                      </div>
                      <div className="flex gap-3 text-xs">
                        <span className="font-semibold text-foreground w-20 shrink-0">Type</span>
                        <span className="text-muted-foreground">{product.productTypeLabel}</span>
                      </div>
                    </div>
                  )}
                </div>
                  ...
                  {reviews.length > 0 ? (
                    <div>
                      ...
                      <a href={`${product.permalink}#reviews`} target="_blank" rel="noopener noreferrer"
                        ...
                        <ExternalLink size={11} /> See all reviews on otakuclothingsl.store
                      </a>
                    </div>
                  ) : (
