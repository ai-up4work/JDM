// app/stores/skye-clothing/[slug]/page.tsx
'use client';
...
import {
  ...
  ShoppingBag, Heart, Star, Check, Truck, RefreshCw,
  ExternalLink, ArrowLeft, AlertCircle, Share2, ZoomIn, Minus, Plus,
} from 'lucide-react';
import type { SkyeProduct } from '@/lib/skye.types';

// ─── Helpers ──────────────────────────────────────────────────────────────────

function fmtPrice(lkr: number) { return `රු${lkr.toLocaleString()}`; }
...
export default function SkyeProductPage() {
  const { slug }                            = useParams<{ slug: string }>();
  const [product,    setProduct]            = useState<SkyeProduct | null>(null);
  ...
  useEffect(() => {
    ...
    update();
    window.addEventListener('skye_cart_updated', update);
    return () => window.removeEventListener('skye_cart_updated', update);
  }, []);

  useEffect(() => {
    if (!slug) return;
    setLoading(true); setError(null);
    fetch(`/api/skye/${encodeURIComponent(slug)}`)
      ...
      <div className="sticky top-0 z-30 bg-background/80 backdrop-blur-md mt-4">
        <div className="max-w-7xl mx-auto h-14 flex items-center justify-between gap-4 px-4 sm:px-10 lg:px-40">
          <div className="flex items-center gap-2 text-xs text-muted-foreground shrink-0 min-w-0">
            ...
            <ChevronRight size={11} className="shrink-0" />
            <Link href="/stores/skye-clothing" className="hover:text-foreground transition-colors font-medium shrink-0">Skye Clothing</Link>
            {product && (
              <>
                <ChevronRight size={11} className="shrink-0" />
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <Link href="/stores/skye-clothing"
          ...
          {error && !loading && (
            <div className="flex items-start gap-3 p-5 rounded-2xl border border-red-200 dark:border-red-900/50 bg-red-50 dark:bg-red-950/20 max-w-lg">
              ...
              <div>
                ...
                <Link href="/stores/skye-clothing" className="text-xs font-semibold text-foreground underline hover:no-underline">← Back</Link>
              </div>
            </div>
          )}
            ...
            <>
                  ...
                  {reviews.length > 0 ? (
                    <div>
                      ...
                      <a href={`${product.permalink}#reviews`} target="_blank" rel="noopener noreferrer"
                        ...
                        <ExternalLink size={11} /> See all reviews on skyeclothing.lk
                      </a>
                    </div>
                  ) : (
              ...
              <div className="mt-14 p-5 rounded-2xl bg-secondary/30 border border-border">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  {[
                    { step: '01', title: 'You order here',      body: 'Add to bag and place your order via WhatsApp. We confirm availability with Skye Clothing.' },
                    { step: '02', title: 'We bulk-buy for you', body: 'We collect orders and purchase from Skye Clothing in bulk — no delivery wait for you.' },
                    { step: '03', title: 'We deliver locally',  body: 'Your item arrives with us and we deliver to your door in your area, same or next day.' },
                  ].map(s => (
