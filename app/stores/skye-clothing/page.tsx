// app/stores/skye-clothing/page.tsx
'use client';
...
import {
  ...
  X, ArrowUpDown, Truck, RefreshCw, Shield, AlertCircle, Search, Tag,
} from 'lucide-react';
import type { SkyeProduct, SkyeApiResponse } from '@/lib/skye.types';

// ─── Categories ───────────────────────────────────────────────────────────────

const CATEGORIES = [
  { key: '',               label: 'All'           },
  { key: 'knit-polos',     label: 'Knit Polos'    },
  { key: 'classic-polos',  label: 'Classic Polos' },
  { key: 'collared-tees',  label: 'Collared Tees' },
  { key: 'shirts',         label: 'Shirts'        },
  { key: 'pattern-tees',   label: 'Pattern Tees'  },
...
const CART_KEY = 'skye_cart';
type CartItem = SkyeProduct & { selectedSize?: string | null; selectedColor?: string | null; qty: number };
...
function writeCart(items: CartItem[]) {
  ...
  window.dispatchEvent(new Event('skye_cart_updated'));
}
...
function ProductCard({ product }: { product: SkyeProduct }) {
  ...
  return (
    <div className="group flex flex-col">
      <Link href={`/stores/skye-clothing/${product.slug}`}
        ...
        <div className="absolute top-3 left-3 flex flex-col gap-1">
          ...
          {!product.inStock && (
            <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-md bg-foreground/80 text-background">
              Sold out
            </span>
          )}
        </div>
      ...
      <Link href={`/stores/skye-clothing/${product.slug}`} className="flex flex-col flex-1 hover:opacity-75 transition-opacity">
        ...
        {product.sizes.length > 0 && (
          <div className="flex items-center gap-1 mb-1.5">
            ...
            {product.sizes.length > 5 && <span className="text-[9px] text-muted-foreground">+{product.sizes.length - 5}</span>}
          </div>
        )}
...
function MiniCart({ items, onClose, onClear }: { items: CartItem[]; onClose: () => void; onClear: () => void }) {
  ...
  const whatsappText = encodeURIComponent(
    `Hi, I'd like to order the following from Skye Clothing:\n\n` +
    items.map(i =>
...
export default function SkyeClothingPage() {
  ...
  const [products,    setProducts]    = useState<SkyeProduct[]>([]);
  ...
  useEffect(() => {
    ...
    window.addEventListener('skye_cart_updated', sync);
    return () => window.removeEventListener('skye_cart_updated', sync);
  }, []);
  ...
  const fetchProducts = useCallback(async (cat: string, pg: number, q: string) => {
    ...
    try {
      ...
      const res = await fetch(`/api/skye?${params}`, { signal: abortRef.current.signal });
      if (!res.ok) {
        const err = await res.json().catch(() => ({}));
        throw new Error(err.error ?? `HTTP ${res.status}`);
      }
      const data: SkyeApiResponse = await res.json();
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="sticky top-0 z-30 bg-background/80 backdrop-blur-md mt-4">
        <div className="max-w-7xl mx-auto h-14 flex items-center justify-between gap-4 px-4 sm:px-10 lg:px-40">
          <div className="flex items-center gap-2 text-xs text-muted-foreground shrink-0">
            ...
            <ChevronRight size={11} />
            <span className="text-foreground font-medium">Skye Clothing</span>
          </div>
      ...
      <div className="w-full min-w-0 px-4 sm:px-10 lg:px-40">
        <div className="max-w-7xl mx-auto py-8 sm:py-12 min-w-0">
          ...
          <div className="mb-10 sm:mb-14">
            <div className="flex items-center gap-3 mb-5">
              <div className="w-10 h-10 rounded-xl bg-foreground flex items-center justify-center shrink-0 overflow-hidden">
                <img 
                    src="/store-icon/skye-clothing.png" 
                    alt="Skye Clothing Logo" 
            ...
            <p className="text-3xl sm:text-4xl font-bold text-foreground tracking-tight leading-tight mb-3 max-w-lg">
              Tees to flatter<br />every angle.
            </p>
            <p className="text-muted-foreground text-sm max-w-md mb-6">
              Sri Lankan menswear — polos, collared tees, shirts and basics made to be worn every day. Live catalogue from Skye Clothing.
            </p>
            ...
            <div className="flex items-center gap-6 flex-wrap">
              {[
                ...
                { icon: <Tag size={13} />,      label: 'LKR prices'           },
              ].map(b => (
          ...
          <div className="flex items-center justify-between gap-3 mb-6 flex-wrap">
            <div className="flex items-center gap-2 flex-1 min-w-0">
              <div className="relative flex-1 sm:flex-none sm:w-64">
                ...
                <input type="text" placeholder="Search Skye Clothing…" value={searchInput}
          ...
          {/* ── Product grid — square aspect to match Skye's photography ── */}
          {loading ? (
          ...
          <div className="mt-16 pt-8 border-t border-border grid grid-cols-1 sm:grid-cols-3 gap-8">
            {[
              { title: 'About Skye Clothing', body: 'Original Skye Clothing products resold locally. Sri Lankan polos, tees and shirts made to flatter — delivered to your door without the wait.' },
              { title: 'Delivery',            body: 'Island-wide shipping in 2–4 working days. Local same-day delivery available. Cash on delivery accepted.' },
              { title: 'How it works',        body: 'Browse here, add to bag, order via WhatsApp. We bulk-buy from Skye Clothing and deliver locally.' },
            ].map(s => (
              <div key={s.title}>
